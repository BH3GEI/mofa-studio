...
impl MoFaFMScreen {
    ...
    pub(super) fn poll_dora_events(&mut self, cx: &mut Cx) {
        ...
        // =====================================================
        // Check for clear signal first (human speaking interrupt)
        let should_clear_audio = if let Some(ref dora) = self.dora_integration {
            dora.shared_dora_state().audio.take_clear_signal()
        } else {
            false
        };

        // Immediately clear audio player buffer if signaled
        if should_clear_audio {
            if let Some(ref player) = self.audio_player {
                player.reset();
                ::log::info!("🔇 Audio buffer cleared (human interrupt)");
            }
        }
        ...
        if let Some(level) = mic_level {
            self.update_mic_level_from_dora(cx, level);
        }

        // Update AEC toggle state if it changed from bridge (sync UI with bridge state)
        ...
        for event in events {
            match event {
                ...
                DoraEvent::DataflowStopped => {
                    ...
                    self.update_mic_level_from_dora(cx, 0.0);

                    // Reset buffer level
                    self.update_buffer_level(cx, 0.0);
                }
