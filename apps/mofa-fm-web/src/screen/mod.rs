//! MoFA.fm Web Screen
//!
//! WebView-based embedded MoFA.fm website
...
live_design! {
    ...
    pub MofaFmWebScreen = {{MofaFmWebScreen}} {
...
fn get_python_path() -> Option<PathBuf> {
    if let Ok(exe_path) = std::env::current_exe() {
        // Check inside app bundle (macOS): .app/Contents/Resources/apps/mofa-fm-web/web
        if let Some(macos_dir) = exe_path.parent() {
            let resources_path = macos_dir
                .parent() // Contents
                .map(|p| p.join("Resources/apps/mofa-fm-web/web"));
        ...
        if let Some(target_dir) = exe_path.parent() {
            if let Some(workspace) = target_dir.parent().and_then(|p| p.parent()) {
                let python_path = workspace.join("apps/mofa-fm-web/python/web");
...
impl PythonServer {
    ...
    fn start(&mut self) -> Result<u16, String> {
        ...
        ::log::info!("Starting MoFA.fm server on port {}", port);
                ...
                if wa.widget_uid == our_uid {
                    match wa.cast() {
                        WebViewAction::Initialized => {
                            ::log::info!("MoFA.fm WebView initialized");
                            self.load_url(cx);
                        }
...
impl MofaFmWebScreen {
    const TARGET_URL: &'static str = "https://mofa.fm/";

    fn toggle_server(&mut self, cx: &mut Cx) {
        self.set_status(cx, "Loading...", 2.0);
        self.view
            .button(ids!(status_bar.start_btn))
            .set_text(cx, "Reload");
        self.load_url(cx);
    }
    ...
    fn load_url(&mut self, cx: &mut Cx) {
        ...
        if let Err(e) = webview.load_url(Self::TARGET_URL) {
