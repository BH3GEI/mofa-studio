//! Hello World (Rust) Screen
//!
//! WebView-based example app with a Rust HTTP server
...
live_design! {
    ...
    pub HelloWorldRustScreen = {{HelloWorldRustScreen}} {
...
fn resolve_static_root() -> Option<PathBuf> {
    if let Ok(exe_path) = std::env::current_exe() {
        // Check inside app bundle (macOS): .app/Contents/Resources/apps/mofa-hello-world-rust/static
        if let Some(macos_dir) = exe_path.parent() {
            let resources_path = macos_dir
                .parent() // Contents
                .map(|p| p.join("Resources/apps/mofa-hello-world-rust/static"));
        ...
        if let Some(target_dir) = exe_path.parent() {
            if let Some(workspace) = target_dir.parent().and_then(|p| p.parent()) {
                let static_path = workspace.join("apps/mofa-hello-world-rust/static");
    ...
    let candidates = [
        "apps/mofa-hello-world-rust/static",
        "../apps/mofa-hello-world-rust/static",
    ];
    ...
    let (status, content_type, body) = match (method, path) {
        ("GET", "/api/info") => {
            let body = serde_json::to_vec(&json!({
                "name": "Hello World Rust Plugin",
                "version": "1.0.0",
                "message": "This is a Rust-powered example plugin!"
            }))
        ...
        ("GET", "/api/time") => {
            let now_ms = SystemTime::now()
                ...
                .as_millis() as u64;
            let body = serde_json::to_vec(&json!({ "time": now_ms }))
                .unwrap_or_else(|_| b"{}".to_vec());
            ("200 OK", "application/json; charset=utf-8", body)
        }
        ("POST", "/api/greet") => {
            ...
            let body = serde_json::to_vec(&json!({
                "greeting": format!("Hello, {}! Welcome to MoFA Studio plugins.", name)
            }))
            .unwrap_or_else(|_| b"{}".to_vec());
            ("200 OK", "application/json; charset=utf-8", body)
        }
        ...
        ("GET", _) => {
            if let Some((bytes, ctype)) = load_static_file(path, assets) {
                ...
                ("404 Not Found", "text/plain; charset=utf-8", body)
            }
        }
        _ => {
...
pub struct HelloWorldRustScreen {
    #[deref]
    view: View,
...
impl Widget for HelloWorldRustScreen {
                ...
                if wa.widget_uid == our_uid {
                    match wa.cast() {
                        WebViewAction::Initialized => {
                            ::log::info!("Hello World Rust WebView initialized");
...
impl HelloWorldRustScreen {
    fn toggle_server(&mut self, cx: &mut Cx) {
        ...
        if is_running {
            ...
            match result {
                Ok(port) => {
                    ::log::info!("Hello World Rust server started on port {}", port);
