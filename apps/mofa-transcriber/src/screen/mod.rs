//! Transcriber Screen
//!
//! WebView-based AI audio/video transcription
...
live_design! {
    ...
    pub TranscriberScreen = {{TranscriberScreen}} {
...
fn get_python_path() -> Option<PathBuf> {
    ...
    if let Ok(exe_path) = std::env::current_exe() {
        // Check inside app bundle (macOS): .app/Contents/Resources/apps/mofa-transcriber/web
        if let Some(macos_dir) = exe_path.parent() {
            let resources_path = macos_dir
                .parent() // Contents
                .map(|p| p.join("Resources/apps/mofa-transcriber/web"));
        ...
        if let Some(target_dir) = exe_path.parent() {
            if let Some(workspace) = target_dir.parent().and_then(|p| p.parent()) {
                let python_path = workspace.join("apps/mofa-transcriber/python/web");
    ...
    // Try relative paths
    let candidates = [
        "apps/mofa-transcriber/python/web",
        "../apps/mofa-transcriber/python/web",
    ];
...
fn save_python_config(python_path: &str) -> Result<(), String> {
    ...
    ::log::info!("Saved Transcriber config: {}", python_path);
    Ok(())
}
...
impl PythonServer {
    ...
    fn start(&mut self) -> Result<u16, String> {
        ...
        ::log::info!("Starting Transcriber server on port {}", port);
...
impl Widget for TranscriberScreen {
                ...
                if wa.widget_uid == our_uid {
                    match wa.cast() {
                        WebViewAction::Initialized => {
                            ::log::info!("Transcriber WebView initialized");
...
impl TranscriberScreen {
    fn start_server(&mut self, cx: &mut Cx) {
        ...
        if is_running {
            ...
            match result {
                Ok(port) => {
                    ::log::info!("Transcriber server started on port {}", port);
...
impl TranscriberScreenRef {
    ...
    pub fn update_dark_mode(&self, cx: &mut Cx, dark_mode: f64) {
        if let Some(mut inner) = self.borrow_mut() {
            ...
            inner.view.button(ids!(status_bar.config_btn)).apply_over(
                cx,
                live! {
                    draw_bg: { dark_mode: (dark_mode) }
            ...
            // Update config panel
            inner.view.view(ids!(config_panel)).apply_over(
                cx,
                live! {
                    draw_bg: { dark_mode: (dark_mode) }
                },
            );
            inner.view.label(ids!(config_panel.python_label)).apply_over(
                cx,
                live! {
                    draw_text: { dark_mode: (dark_mode) }
                },
            );
            inner.view.text_input(ids!(config_panel.python_input)).apply_over(
                cx,
                live! {
                    draw_bg: { dark_mode: (dark_mode) }
                    draw_text: { dark_mode: (dark_mode) }
            ...
            inner.view.button(ids!(config_panel.save_btn)).apply_over(
                cx,
                live! {
                    draw_bg: { dark_mode: (dark_mode) }
                    draw_text: { dark_mode: (dark_mode) }
