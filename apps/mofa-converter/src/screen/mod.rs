//! Content Converter Screen
//!
//! WebView-based content converter for audio, video, and text
...
live_design! {
    ...
    pub ConverterScreen = {{ConverterScreen}} {
...
fn get_python_path() -> Option<PathBuf> {
    if let Ok(exe_path) = std::env::current_exe() {
        // Check inside app bundle (macOS)
        if let Some(macos_dir) = exe_path.parent() {
            let resources_path = macos_dir
                .parent()
                .map(|p| p.join("Resources/apps/mofa-converter"));
            if let Some(ref path) = resources_path {
                if path.join("python/web/app.py").exists() {
                    return Some(path.join("python/web"));
                }
            }
        }
        ...
        if let Some(target_dir) = exe_path.parent() {
            if let Some(workspace) = target_dir.parent().and_then(|p| p.parent()) {
                let python_path = workspace.join("apps/mofa-converter/python/web");
    ...
    let candidates = [
        "apps/mofa-converter/python/web",
        "../apps/mofa-converter/python/web",
    ];
...
impl PythonServer {
    ...
    fn start(&mut self) -> Result<u16, String> {
        ...
        ::log::info!("Starting Converter server on port {}", port);
...
impl Widget for ConverterScreen {
                ...
                if wa.widget_uid == our_uid {
                    match wa.cast() {
                        WebViewAction::Initialized => {
                            ::log::info!("Converter WebView initialized");
...
impl ConverterScreen {
    fn toggle_server(&mut self, cx: &mut Cx) {
        ...
        if is_running {
            ...
            match result {
                Ok(port) => {
                    ::log::info!("Converter server started on port {}", port);
